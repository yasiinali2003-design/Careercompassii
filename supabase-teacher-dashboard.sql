...
CREATE TABLE IF NOT EXISTS classes (
  ...
  class_token TEXT NOT NULL UNIQUE, -- 22+ char random token for public access
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Pins table (student PINs)
CREATE TABLE IF NOT EXISTS pins (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  class_id UUID NOT NULL REFERENCES classes(id) ON DELETE CASCADE,
  pin TEXT NOT NULL UNIQUE, -- 4-6 alphanumeric PIN
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS results (
  ...
  result_payload JSONB NOT NULL, -- { top_careers, dimension_scores, etc. }
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Allow anonymous reads by class_token (for public link)
-- This will be handled by RPC function instead

-- ========== RLS POLICIES FOR PINS ==========

-- Only class owner can manage pins
...
-- Allow anonymous inserts for students taking tests
CREATE POLICY "Students can insert pins during test"
  ON pins
  FOR INSERT
  WITH CHECK (true); -- Will be validated by API/Edge function

-- ========== RLS POLICIES FOR RESULTS ==========

-- Only class owner can view results
CREATE POLICY "Class owners view results"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM classes c 
      WHERE c.id = results.class_id 
      AND c.teacher_id = auth.uid()
    )
  );

-- Allow anonymous inserts (students submitting tests)
CREATE POLICY "Students can submit results"
  ON results
  FOR INSERT
  WITH CHECK (true); -- Validated by API/Edge function with rate limiting

-- Disable updates and deletes for results (immutable history)
CREATE POLICY "No updates or deletes on results"
  ON results
  FOR UPDATE
  USING (false);

CREATE POLICY "No deletes on results"
  ON results
  FOR DELETE
  USING (false);
...
-- Get class info by token (public, no teacher info)
...
-- Allow anonymous users to call public functions
...
-- Allow service role (API) to insert results
