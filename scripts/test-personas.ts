...
function runPersonaTest(persona: TestPersona, cohort: Cohort, subCohort?: string) {
  console.log(`\n${'='.repeat(70)}`);
  ...
  const { dimensionScores, detailedScores } = computeUserVector(testAnswers, cohort, subCohort);

  console.log(`\n📊 DIMENSION SCORES:`);
  console.log(`  Interests: ${(dimensionScores.interests * 100).toFixed(0)}%`);
  console.log(`  Values: ${(dimensionScores.values * 100).toFixed(0)}%`);
  console.log(`  Workstyle: ${(dimensionScores.workstyle * 100).toFixed(0)}%`);
  console.log(`  Context: ${(dimensionScores.context * 100).toFixed(0)}%`);

  console.log(`\n🎯 TOP SUBDIMENSIONS (Interests):`);
  const interestEntries = Object.entries(detailedScores.interests || {})
    .sort(([,a], [,b]) => (b as number) - (a as number))
    .slice(0, 5);
  interestEntries.forEach(([key, val]) => {
    console.log(`  ${key}: ${((val as number) * 100).toFixed(0)}%`);
  });

  console.log(`\n💼 TOP SUBDIMENSIONS (Workstyle):`);
  const workstyleEntries = Object.entries(detailedScores.workstyle || {})
    .sort(([,a], [,b]) => (b as number) - (a as number))
    .slice(0, 5);
  workstyleEntries.forEach(([key, val]) => {
    console.log(`  ${key}: ${((val as number) * 100).toFixed(0)}%`);
  });

  // Get career recommendations
  const careers = rankCareers(testAnswers, cohort, 5, undefined, subCohort);

  console.log(`\n🏆 TOP 5 CAREER RECOMMENDATIONS:`);
  careers.forEach((career, i) => {
    const matchesExpected = persona.expectedCareers.some(exp =>
      career.slug.toLowerCase().includes(exp.toLowerCase()) ||
      exp.toLowerCase().includes(career.slug.toLowerCase())
    );
    const marker = matchesExpected ? '✅' : '⚠️';
    console.log(`  ${i+1}. ${marker} ${career.title || career.slug} (${career.category}) - Score: ${career.overallScore}%`);
    if (career.reasons && career.reasons.length > 0) {
      console.log(`     Reason: ${career.reasons[0]}`);
    }
  });

  // Get education path
  const educationPath = calculateEducationPath(testAnswers, cohort);
  console.log(`\n📚 EDUCATION PATH:`);
  if (educationPath) {
    console.log(`  Primary: ${educationPath.primary}`);
    console.log(`  Confidence: ${educationPath.confidence}`);
    console.log(`  Reasoning: ${educationPath.reasoning}`);
  } else {
    console.log(`  Path: Not calculated for this cohort (NUORI)`);
  }
  ...
  const careerMatches = careers.filter(c =>
    persona.expectedCareers.some(exp =>
      c.slug.toLowerCase().includes(exp.toLowerCase()) ||
      exp.toLowerCase().includes(c.slug.toLowerCase())
    )
  ).length;
...
async function main() {
  ...
  for (const persona of YLA_PERSONAS) {
    results.push(runPersonaTest(persona, 'YLA'));
  }
  ...
  for (const persona of NUORI_PERSONAS) {
    results.push(runPersonaTest(persona, 'NUORI'));
  }
  ...
  for (const persona of TASO2_LUKIO_PERSONAS) {
    results.push(runPersonaTest(persona, 'TASO2', 'LUKIO'));
  }
  ...
  for (const persona of TASO2_AMIS_PERSONAS) {
    results.push(runPersonaTest(persona, 'TASO2', 'AMIS'));
  }
