import { generateUserProfile, computeUserVector } from './lib/scoring/scoringEngine';
...
// Test Aleksi using generateUserProfile (exactly like the test file)
console.log("=== Aleksi via generateUserProfile ===");
...
console.log("Top 3 categories:", aleksiProfile.categoryAffinities?.slice(0, 3).map((c: any) => `${c.category}(${c.score}%)`).join(', '));

// Directly call calculateCategoryAffinities
console.log("\n=== Aleksi via direct calculateCategoryAffinities ===");
...
console.log("Top 3 categories:", aleksiAffinities.slice(0, 3).map((c: any) => `${c.category}(${c.score}%)`).join(', '));
