/**
 * NUORI QUESTION POOL VALIDATION TEST
 * Tests question set selection, mapping, and scoring accuracy for NUORI
 */
...
function getAvailableSets(cohort) {
  const totalSets = 3; // NUORI has 3 sets
  const usedSets = getUsedSets(cohort);
...
console.log('=== NUORI QUESTION POOL TEST ===\n');

// Test 1: Initial State
console.log('TEST 1: Initial State');
localStorage.clear();
localStorage.removeItem(getStorageKey('NUORI'));
...
const setsUsed = [];
for (let i = 0; i < 4; i++) {
  const set = selectQuestionSet('NUORI');
  setsUsed.push(set);
  markSetAsUsed('NUORI', set);
  const used = getUsedSets('NUORI');
