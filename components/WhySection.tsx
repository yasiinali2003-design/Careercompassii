...
export function WhySection() {
  return (
    <AnimatedSection className="max-w-5xl mx-auto px-6 py-16 md:py-20">
      {/* Header */}
      <header className="mb-10">
        ...
        <p className="text-sm text-urak-text-secondary md:text-base">
        ...
        {reasons.map((item, index) => {
          ...
          return (
            <AnimatedCard
              key={item.id}
              delay={index * 0.08}
              className="group relative overflow-hidden rounded-3xl border border-white/5 bg-white/5 bg-gradient-to-b from-white/[0.05] to-white/0 backdrop-blur-xl px-5 py-6 md:px-6 md:py-7 shadow-[0_18px_50px_rgba(0,0,0,0.45)] transition-transform duration-300 ease-out hover:-translate-y-1"
            >
              {/* Hover Glow */}
              ...
              {/* Icon Bubble */}
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-urak-bg/60 ring-1 ring-white/10 shadow-md mb-4 relative z-10">
                <Icon className="h-5 w-5 text-urak-accent-blue" />
              </div>

              {/* Title */}
              <h3 className="text-lg font-semibold text-urak-text-primary mb-2 relative z-10">
                {item.title}
              </h3>

              {/* Body */}
              <p className="mt-2 text-sm text-urak-text-secondary leading-relaxed relative z-10">
                {item.body}
              </p>
            </AnimatedCard>
          );
