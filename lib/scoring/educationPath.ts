...
function generateYLAReasoning(
  ...
  if (primary === 'lukio') {
    if (confidence === 'high') {
      reasoning = `Profiilistasi välittyy, että arvostat monipuolista oppimista ja haluat pitää vaihtoehdot auki tulevaisuutta varten. Sinussa on vahva uteliaisuus eri aineita kohtaan ja halu ymmärtää asioita syvällisesti. Lukiossa pääset tutustumaan laajasti eri aineisiin matematiikasta taiteeseen ja luonnontieteistä kieliin. Samalla kehität vahvaa opiskelutaitoa ja itsenäistä ajattelua. Lukio sopii sinulle erityisen hyvin, koska haluat rakentaa vahvan perustan jatko-opinnoille ja antaa itsellesi aikaa löytää oma alasi rauhassa.`;
    } else if (confidence === 'medium') {
      reasoning = `Profiilistasi välittyy, että nautit teoreettisesta oppimisesta ja monipuolisesta opiskelusta. Sinussa on halu ymmärtää asioita laajasti ja kehittää vahvoja opiskelutaitoja. Lukio voi olla kiinnostava vaihtoehto, jos koet tärkeäksi monipuolista oppimista ja mahdollisuutta tutustua eri aineisiin. Lukiossa voit rakentaa vahvan perustan, joka avaa monia mahdollisuuksia tulevaisuudessa.`;
    } else {
      reasoning = `Profiilistasi välittyy, että arvostat monipuolisuutta ja haluat pitää vaihtoehdot auki. Lukio on hyvä valinta, jos haluat rakentaa vahvan perustan jatko-opinnoille ja antaa itsellesi aikaa löytää oma alasi. Lukiossa pääset tutustumaan laajasti eri aineisiin ja kehittämään opiskelutaitojasi.`;
    }
  } else if (primary === 'ammattikoulu') {
    if (confidence === 'high') {
      reasoning = `Profiilistasi välittyy, että nautit käytännön tekemisestä ja oppimisesta tekemällä. Sinussa on vahva halu nähdä konkreettisia tuloksia ja oppia taitoja, joita voit heti käyttää käytännössä. Ammattikoulussa pääset heti käytännön töihin ja opit tietyn ammatin taidot konkreettisesti työtilanteissa. Ammatillinen koulutus sopii sinulle erityisen hyvin, koska haluat oppia konkreettisen ammatin taidot ja aloittaa työelämän suhteellisen pian. Saat myös arvokasta työkokemusta jo opiskelun aikana harjoitteluissa, mikä helpottaa työllistymistä valmistumisen jälkeen.`;
    } else if (confidence === 'medium') {
      reasoning = `Profiilistasi välittyy, että nautit käytännön tekemisestä ja konkreettisesta oppimisesta. Sinussa on halu oppia taitoja, joita voit käyttää työssä. Ammattikoulu voi olla kiinnostava vaihtoehto, jos koet tärkeäksi käytännönläheistä oppimista ja mahdollisuutta päästä tekemään konkreettisia töitä. Ammattikoulussa pääset työskentelemään oman alasi parissa ja opit käytännön työelämän taitoja.`;
    } else {
      reasoning = `Profiilistasi välittyy, että arvostat käytännön tekemistä ja konkreettisia taitoja. Ammatillinen koulutus on hyvä valinta, jos haluat oppia konkreettisen ammatin taidot ja päästä nopeasti töihin. Ammattikoulussa pääset heti tekemään käytännön töitä ja saat konkreettista kokemusta työelämästä. Kannattaa tutustua eri aloihin ja miettiä, mikä ala voisi kiinnostaa sinua erityisesti.`;
    }
  } else { // kansanopisto
    if (confidence === 'high') {
      reasoning = `Profiilistasi välittyy, että tarvitset aikaa löytää oma polkusi. Sinussa on uteliaisuus eri asioita kohtaan, mutta vielä epävarmuus siitä, mikä ala tai ammatti sinua kiinnostaisi eniten. Kansanopistossa voit tutustua eri aloihin rauhassa, kasvaa ihmisenä ja selkiyttää omaa suuntaasi ennen seuraavaa vaihetta. Kansanopisto sopii sinulle erityisen hyvin, koska arvostat aikaa miettiä tulevaisuuttasi ilman paineita tai kiirettä. Saat tilaa löytää oma polkusi ja mahdollisuuden löytää intohimosi.`;
    } else {
      reasoning = `Profiilistasi välittyy, että olet vielä miettimässä tulevaisuuden suuntaa. Lukio ja ammattikoulu ovat molemmat mahdollisia vaihtoehtoja. Voit myös harkita kansanopistovuotta, jossa voit tutustua eri aloihin ja selkiyttää omaa suuntaasi rauhassa. Kansanopisto antaa sinulle aikaa miettiä ja löytää oman polkusi ilman kiirettä päätösten tekemiseen.`;
    }
  }
...
function generateTASO2Reasoning(
  ...
  if (primary === 'yliopisto') {
    if (confidence === 'high') {
      reasoning = `Vastaustesi perusteella yliopisto-opinnot sopivat sinulle erittäin hyvin. Profiilisi osoittaa vahvaa analyyttistä ajattelua, tutkimusmielisyyttä ja syvällistä oppimishalua. Yliopistossa opiskelet tutkintoon johtavalla linjalla ja saat laajaa teoreettista tietämystä sekä kriittisen ajattelun taitoja. Tämä polku sopii erityisesti, jos haluat syventää tietämystäsi tietyssä alassa ja harkitset mahdollisesti jatko-opintoja tai tutkijanuraa.
...
Yliopisto-opinnot kestävät tyypillisesti 3–5 vuotta ja tarjoavat hyvät valmiudet monipuoliseen työelämään. Työmarkkinanäkymät ovat yleisesti hyvät, erityisesti teknologia- ja terveysaloilla.`;
    ...
    if (confidence === 'high') {
      reasoning = `Vastaustesi perusteella ammattikorkeakoulu-opinnot sopivat sinulle erittäin hyvin. Profiilisi osoittaa vahvaa käytännönläheisyyttä, konkreettista ongelmanratkaisukykyä ja halua oppia työelämään soveltuvia taitoja. Ammattikorkeakouluissa opiskelutapa on käytännönläheinen ja työelämäprojekteja on runsaasti, mikä auttaa kehittämään työelämässä tarvittavia taitoja jo opiskelun aikana.
    ...
    switch (path) {
      case 'yliopisto':
        return {
          ...
          applicationInfo: 'Haku yhteishaussa Opintopolussa keväisin. Useimmissa ohjelmissa pääsykoe tai todistusvalinta. Valmistaudu pääsykokeisiin ajoissa, sillä kursseja ja materiaaleja on saatavilla.',
